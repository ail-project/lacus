...
api = Api(app, title="Lacus API", description="API to query lacus.", version=version("lacus"))
...
http_creds_model = api.model(
    "HttpCredentialtModel", {"username": fields.String(), "password": fields.String()}
)

viewport_model = api.model("ViewportModel", {"width": fields.Integer(), "height": fields.Integer()})

geolocation_model = api.model(
    "GeolocalisationModel", {"longitude": fields.Float(), "latitude": fields.Float()}
)
...
submit_fields_post = api.model(
    "SubmitFieldsPost",
    {
        ...
        "document": fields.String(
            description="A base64 encoded document, it can be anything a browser can display."
        ),
        "document_name": fields.String(description="The name of the document."),
        "depth": fields.Integer(
            description="Depth of the capture, based on te URLs that can be found in the rendered page."
        ),
        "rendered_hostname_only": fields.Boolean(
            description="If depth is >0, which URLs are we capturing (only the ones with the same hostname as the rendered page, or all of them.)"
        ),
        "browser": fields.String(
            description="Use this browser. Must be chromium, firefox or webkit.", example=""
        ),
        "device_name": fields.String(
            description="Use the pre-configured settings for this device. Get a list from /json/devices.",
            example="",
        ),
        "user_agent": fields.String(description="User agent to use for the capture", example=""),
        "proxy": fields.Url(
            description="Proxy to use for the capture. Format: [scheme]://[username]:[password]@[hostname]:[port]",
            example="",
        ),
        "general_timeout_in_sec": fields.Integer(
            description="General timeout for the capture. It will be killed regardless the status after that time."
        ),
        "cookies": fields.String(
            description="JSON export of a list of cookies as exported from an other capture",
            example="",
        ),
        "headers": fields.String(
            description="Headers to pass to the capture",
            example="Accept-Language: en-US;q=0.5, fr-FR;q=0.4",
        ),
        "http_credentials": fields.Nested(
            http_creds_model, description="HTTP Authentication settings"
        ),
        "geolocation": fields.Nested(
            geolocation_model, description="The geolocalisation of the browser"
        ),
        ...
        "force": fields.Boolean(
            description="Force a capture, even if the same one was already done recently"
        ),
        "recapture_interval": fields.Integer(
            description="The nimomal interval to re-trigger a capture, unless force is True"
        ),
...
@api.doc(
    description="Get the status of a capture.", params={"capture_uuid": "The UUID of the capture"}
)
...
@api.doc(
    description="Get the result of a capture.", params={"capture_uuid": "The UUID of the capture"}
)

...
def _get_upstream_target(request: web.Request) -> tuple[str, str]:
    ...
    if session_metadata.get('capture_requested_at') or status_int in (
    ...
    return capture_uuid, socket_path


def _get_api_base_url() -> str:
    host = get_config('generic', 'website_listen_ip') or '127.0.0.1'
    if host in LOCALHOSTS:
        host = '127.0.0.1'
    port = int(get_config('generic', 'website_listen_port') or 7100)
...
async def _proxy_api_request(request: web.Request, upstream_path: str) -> web.Response:
    ...
    upstream_url = f'{_get_api_base_url()}{upstream_path}'
...
async def _proxy_http(request: web.Request) -> web.StreamResponse:
    _, socket_path = _get_upstream_target(request)
